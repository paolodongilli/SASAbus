	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView == null) {
			convertView = LayoutInflater.from(this.getContext()).inflate(
					R.layout.parking_free_slots, parent, false);
		}
		ParkingData data = this.getItem(position);

		TextView nameView = (TextView) convertView.findViewById(R.id.parkName);
		TextView slotsView = (TextView) convertView
				.findViewById(R.id.parkSlots);
		ProgressBar progressBar = (ProgressBar) convertView
				.findViewById(R.id.parkProgressBar);
		TextView adressView = (TextView) convertView.findViewById(R.id.parkAdress);
		TextView phoneView = (TextView) convertView.findViewById(R.id.parkPhone);

		nameView.setText(data.name);
		slotsView.setText("" + (data.tot - data.free) + "/" + data.tot);
		progressBar.setMax(data.tot);
		progressBar.setProgress(data.tot - data.free);
		adressView.setText(data.adress);
